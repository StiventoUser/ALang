    ...
    public sealed class CompilationArguments
    {
        public bool HasCommand(string command, bool checkRequired = true)
        {
            ...
            if (!hasCommand)
            {
                ...
                {
                    Console.WriteLine("Command {0} is expected" +
                                      (!string.IsNullOrEmpty(defaultInfo.ArgsInfo)
                                          ? (" with arguments: " + defaultInfo.ArgsInfo)
                                          : ""), command);
                    if (checkRequired)
                    {
        ...
        private List<CommandInfo> m_commands = new List<CommandInfo>()
        {
            ...
            new CommandInfo
            {
                ...
                Required = true,
                ArgCount = -1,
                ArgsInfo = "file1.alangs path/to/file/file2 file3 ..."
            },
            new CommandInfo
            {
                FullName = "output",
                ...
                Required = true,
                ArgCount = 1,
                ArgsInfo = "path/to/file/file.alang"
            }
        };
