...
{
    /// <summary>
    /// Stores constant value as a string.
    /// </summary>
    public sealed class StringToValue
    {
        ...
        {
            Compilation.Assert(!string.IsNullOrEmpty(m_typeName), "Can't cast unknown type to value of type" + cSharpType,
                -1);
            Compilation.Assert(!string.IsNullOrEmpty(StringVal), "Can't cast empty string to value of type" + cSharpType,
                -1);

            var lang = LanguageSymbols.Instance;
            if (lang.GetDefaultTypeId(lang.GetTypeNameByCSharpTypeName(cSharpType)) != m_typeId)
            {
                Compilation.WriteError(string.Format("Can't cast from type {0} to CSharp type {1}", m_typeName, cSharpType),
                    -1);
            }
        }
